<?php
if( ! class_exists( 'Register_Settings_API_Cookie_message' ) ) {
	class Register_Settings_API_Cookie_message {
		/**
		...
		function init_settings() {
			if( is_admin() ) {
				$this->settings_array = apply_filters('register_settings_api_cookie_message', array());
				...
				if( ! empty( $_GET['callback'] ) ) {
					...
					if( ! empty( $nonce ) ) {
						...
						if( function_exists( $_GET['callback'] ) ) {
							...
							wp_redirect( $url );
							die;
						} elseif( $_GET['callback'] == 'rsa_delete_settings' && method_exists( $this, 'delete_settings' ) ) {
		...
		/**
		 * Delete settings on the current page
		 *
		 * @since 1.5
 		 */
		function delete_settings() {
			...
			/*
			Behöver page slug och field slug
			Skapa funktion som loopar så det blir som $args
			Hänta option från page slug
			*/
			//delete_option();
			//update_option('rsa-message', 'Settings deleted');
			$url = admin_url('options-general.php?page=' . $_GET['page']);
			//wp_redirect( $url );
		}
			...
			if( ! empty( $args['type'] ) ) {
				switch( $args['type'] ) {
					...
					case 'delete':
						$warning_message = ( ! empty( $args['warning-message'] ) ) ? $args['warning-message'] : 'Unsaved settings will be lost. Continue?';
						$warning = ( ! empty( $args['warning'] ) ) ? ' onclick="return confirm(' . "'" . $warning_message . "'" . ')"' : '';
						$label = ( ! empty( $args['label'] ) ) ? $args['label'] : 'Delete settings';
						$complete_url = wp_nonce_url( admin_url('options-general.php?page=' . $_GET['page'] . '&callback=rsa_delete_settings' ) );
						?>
						<a href="<?php echo $complete_url; ?>" class=""<?php echo $warning; ?>><?php echo $label; ?></a>
						<?php
						break;
				}
			}
	...
	$settings_api_cookie_message = new Register_Settings_API_Cookie_message();
}